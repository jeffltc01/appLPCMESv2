...
export function GaugesSetupPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      setRows(await setupApi.listGauges());
    } catch {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        await setupApi.updateGauge(editing.id, payload);
      } else {
        await setupApi.createGauge(payload);
      }
      setDialogOpen(false);
      await load();
    } catch (err: unknown) {
      ...
      setError(body?.message ?? body?.detail ?? "Failed to save gauge.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (row: LookupOptionAdmin) => {
    if (!window.confirm(`Delete gauge '${row.displayName}'?`)) return;
    setError(null);
    try {
      await setupApi.deleteGauge(row.id);
      await load();
    } catch (err: unknown) {
      const apiError = err as ApiError;
      const body = apiError.body as { message?: string; detail?: string } | undefined;
      setError(body?.message ?? body?.detail ?? "Failed to delete gauge.");
    }
  };
    ...
    <div className={styles.page}>
      <main className={styles.main}>
        ...
        <header className={styles.headerBar}>
          <Title1 style={{ color: "#ffffff" }}>Gauge Maintenance</Title1>
          <div className={styles.headerActions}>
            <Button appearance="secondary" onClick={() => navigate("/")}>Home</Button>
            <Button appearance="primary" onClick={openCreate}>Add Gauge</Button>
          </div>
        </header>
      ...
      <Dialog open={dialogOpen} onOpenChange={(_, data) => setDialogOpen(data.open)}>
        <DialogSurface>
          <DialogBody>
            <DialogTitle>{editing ? "Edit Gauge" : "Add Gauge"}</DialogTitle>
            <DialogContent>
